...
public class TrackNRestartPlugin extends JDBCSupport implements SchedulerPlugin, JobListener, SchedulerListener {
	...
	public void start() {
		try {
			getLog().info("----------------- Start scheduler "+scheduler.getSchedulerName());
	...
	public void shutdown() {
		try {
			getLog().info("----------------- Shutdown scheduler "+scheduler.getSchedulerName());
		} catch (SchedulerException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void jobAdded(JobDetail jobDetail) {
		getLog().debug("Job "+jobDetail.getKey()+" added.");
		JobKey jobKey = null;
		try {
