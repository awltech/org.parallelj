...
privileged aspect LaunchManagement { 
	...
	static privileged public aspect LaunchAdapter percflow(launchInstance() ) {
		...
		private void initializeExecutors(LaunchImpl<?> launch) {
			...
			if (configuration.getExecutorServices() != null && configuration.getExecutorServices().getExecutorService() != null) {
				for(CExecutor cExecutor:configuration.getExecutorServices().getExecutorService()) {
